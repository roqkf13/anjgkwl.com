Claude Code 작업 지시서 — 인증 게이트웨이(auth.anjgkwl.com) 분리 배포
...
- 현재 main.py 하나로 api.anjgkwl.com에 배포 중. apps/ 하위 영화 제목 앱들이 비즈니스 라우터.
- 목표: 같은 코드베이스에서 엔트리포인트를 분리해 auth.anjgkwl.com(인증 전용)과 api.anjgkwl.com(비즈니스)을 별도 컨테이너로 운영.
...
- 어떤 서비스에도 docker-compose.yaml에 ports: 매핑을 추가하지 않는다.
...
# 쿠키 설정 — auth 발급 시 사용
COOKIE_KWARGS = dict(
    domain=".anjgkwl.com", secure=True, httponly=True, samesite="lax",
)
...
```
ingress:
  - hostname: auth.anjgkwl.com
    service: http://auth:9000
  - hostname: api.anjgkwl.com
...
```
cloudflared tunnel route dns <터널이름> auth.anjgkwl.com
