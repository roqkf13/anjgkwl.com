...
import { Upload, FolderOpen, Loader2, CheckCircle2, AlertCircle, FileText } from "lucide-react";
...
export function RagDocumentUploadPanel() {
  const inputRef = useRef<HTMLInputElement>(null);
  const [file, setFile] = useState<File | null>(null);
  const [ui, setUi] = useState<UploadUiState>(initialUploadUi);
  const [result, setResult] = useState<RagUploadResponse | null>(null);
  ...
  const pickFile = useCallback((f: File | null) => {
    ...
    if (!hasAllowedExtension(f.name)) {
      ...
      setUi((prev) => ({
        ...prev,
        feedback: { kind: "error", text: "pdf, txt, md 파일만 선택할 수 있습니다." },
      }));
      return;
    }
    setFile(f);
  ...
  const upload = async () => {
    ...
    try {
      ...
      setUi((prev) => ({
        ...prev,
        feedback: { kind: "success", text: data.message },
      }));
    } catch {
      setUi((prev) => ({
        ...prev,
        feedback: {
          kind: "error",
          text: "서버 업로드에 실패했습니다. 백엔드 POST /rag/upload 로 전송됩니다.",
        },
      }));
    } finally {
  ...
  return (
    <>
      ...
      <p className="text-sm text-gray-500 text-center mb-10 max-w-md">
        pdf, txt, md 문서만 업로드할 수 있습니다. 아래 영역에 놓거나, 버튼으로 선택한 뒤 업로드하세요.
      </p>
      ...
      <div
        role="button"
        tabIndex={0}
        aria-label="문서 파일을 끌어다 놓거나 클릭하여 선택"
        onKeyDown={(e) => {
      ...
      >
        ...
        <p className="text-sm text-gray-500">
          pdf, txt, md 문서를 여기에 끌어다 놓거나, 이 영역을 클릭해 선택하세요.
        </p>
      </div>
      ...
      {result && (
        <dl className="mt-4 grid max-w-md grid-cols-[auto_1fr] gap-x-3 gap-y-1 text-xs text-gray-500">
          <dt className="font-medium">파일명</dt>
          <dd>{result.filename}</dd>
          <dt className="font-medium">타입</dt>
