...
export function VisionUploadPanel() {
  ...
  const [result, setResult] = useState<VisionUploadResponse | null>(null);
  ...
  const upload = async () => {
    ...
    try {
      const res = await fetch(`${apiBaseUrl}/vision/upload`, {
        method: "POST",
        body,
      });
      if (!res.ok) {
      ...
      const data = (await res.json()) as VisionUploadResponse;
      setResult(data);
      setUi((prev) => ({
        ...prev,
      ...
      setUi((prev) => ({
        ...
        feedback: {
          kind: "error",
          text: "서버 업로드에 실패했습니다. 백엔드 POST /vision/upload 로 전송됩니다.",
        },
      }));
    } finally {
  ...
  return (
    <>
      ...
      <p className="text-sm text-gray-500 text-center mb-10 max-w-md">
        jpg, png 이미지만 업로드할 수 있습니다. 아래 영역에 놓거나, 버튼으로 선택한 뒤 업로드하세요.
      </p>
      ...
      <div
        role="button"
        tabIndex={0}
        aria-label="이미지 파일을 끌어다 놓거나 클릭하여 선택"
        onKeyDown={(e) => {
