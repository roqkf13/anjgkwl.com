...
### Hermes MCP 서버 등록 — user 스코프 대신 project 스코프(`.mcp.json`)로 통일
처음엔 `claude mcp add hermes -s user`로 이 PC의 `~/.claude.json`(user 스코프)에 등록했다가, 곧바로 저장소의 `.mcp.json`(project 스코프)에도 동일하게 등록한 뒤, user 스코프 쪽을 삭제해서 `.mcp.json` 하나로 정리함.
**왜:** user 스코프는 PC 로컬 파일이라 git으로 동기화되지 않음 — 다른 PC로 옮기면 등록 자체가 사라짐. 반면 `.mcp.json`은 git으로 커밋되어 어느 PC에서든 `git pull`하면 등록 내용이 보임(단, 명령어 안의 WSL 배포판 이름은 PC별로 다를 수 있어 조정이 필요할 수 있음 — 이건 감수하기로 함).

### `_notes/` 구조를 가볍게 유지 — feature_list.json / 자동화된 Ralph Loop는 도입하지 않음
장시간 에이전트 하네스 관련 글(wikidocs 8장, Anthropic "Effective harnesses for long-running agents" 요약)을 검토한 뒤, 거기 나온 `progress.md`/`feature_list.json`(JSON, `passes: false`로 시작해 검증 후에만 true로 바꾸는 구조)/Initializer-Coding 에이전트 분리 같은 전체 장치를 그대로 들여오지 않기로 함. 대신 `MEMORY.md`(결정 로그) + `ERRORS.md`(실패 노트) + `HANDOFF.md`(세션 인계 스냅샷) 세 파일로 단순화.
**왜:** 그 글의 장치들은 사람 개입 없이 도는 완전 자율 장시간 루프를 전제로 한 것인데, 이 저장소 작업은 매 턴 사용자가 방향을 확인해주는 상호작용형이라 그 정도 무게의 장치가 필요 없다고 판단함. "완료 전 검증" 원칙 자체는 이미 이 CLAUDE.md 4절(목표 중심 실행과 검증 가능성)이 다루고 있어 중복이기도 함.
**주의:** 나중에 사람 개입 없이 긴 작업(예: 대규모 마이그레이션)을 맡길 일이 생기면, 그 작업 전용으로 `feature_list.json` 같은 무거운 장치를 그때 만드는 걸 권장 — 지금 이 구조에 끼워 넣지 말 것.
